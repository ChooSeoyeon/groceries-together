  ...
  return (
    <Drawer open={open} onOpenChange={setOpen}>
      <DrawerTrigger asChild>
        <Button size="lg" className="rounded-full fixed bottom-6 right-6 h-14 w-14 shadow-lg z-50">
          <Plus className="w-6 h-6" />
        </Button>
      </DrawerTrigger>
      <DrawerContent className="px-4 pb-8">
        <DrawerHeader className="px-0">
          <DrawerTitle className="text-left">물건 추가</DrawerTitle>
        </DrawerHeader>
        ...
        <div className="space-y-4">
          ...
          <Button onClick={handleAdd} className="w-full rounded-full" size="lg" disabled={!name.trim()}>
            추가하기
          </Button>
        </div>
      </DrawerContent>
